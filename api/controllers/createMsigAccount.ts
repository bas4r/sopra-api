...
import { DEFAULT_CHAIN_URL, SOPRA_RECOVERY_ADDRESS, SOPRA_RECOVERY_PRIVATE_KEY } from '../../constants'
...
import { calculateProxyAddress, getCreateProxyTransaction, sendTransaction, setupInitilaizerAction, signTransaction } from '../../gnosisSafe/helpers'
import { GnosisMultisigCreateOptions } from '../../gnosisSafe/models'

export const createMsigAccount = async (req: any, res: any, next: any): Promise<Response> => {
  try {
    ...
    const createMsigTransaction = await getCreateProxyTransaction(msigOptions, chainUrl)

    const signedHash = await signTransaction(createMsigTransaction, privateKey, chainUrl)
